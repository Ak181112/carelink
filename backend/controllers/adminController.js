...
const getDashboardStats = async (req, res, next) => {
  try {
    ...
    const totalClients = await User.countDocuments({ role: "family_member" });
    const pendingApplications = await CaretakerApplication.countDocuments({ status: "pending" });
    const approvedApplications = await CaretakerApplication.countDocuments({ status: "approved" });
    const rejectedApplications = await CaretakerApplication.countDocuments({ status: "rejected" });
...
const approveApplication = async (req, res, next) => {
  try {
    const application = await CaretakerApplication.findById(req.params.id).populate("caretakerId", "name email");
    ...
    await Notification.create({
      ...
      message: "Congratulations! Your caretaker application has been approved. You can now receive care requests.",
...
const rejectApplication = async (req, res, next) => {
  try {
    ...
    await Notification.create({
      ...
      message: `Your caretaker application has been reviewed. ${req.body.note ? `Note: ${req.body.note}` : "Please update your profile and reapply."}`,
