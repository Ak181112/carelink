...
const freshStages = () =>
  STAGES.map(([key, label]) => ({ key, label, status: "not_started" }));
const getBooking = (id) =>
  Booking.findById(id)
    .populate("parentId")
...
async function resolveCaretaker(inputId) {
  ...
  let profile = null;
  try {
    profile = await CaretakerProfile.findOne({
      _id: inputId,
...
async function calculateQuote(req, res, next) {
  try {
    ...
    if (
      ...
      return res
        .status(400)
        .json({ success: false, message: "Select a valid pickup location" });
    ...
    const route = await computeRoadDistance(
      ...
      hospital.location,
      {
        allowFallback:
          String(
            process.env.REQUIRE_GOOGLE_ROUTES_FOR_PRICING ?? "true",
          ).toLowerCase() !== "true",
      },
    );
    const requireGoogleRoutes =
      String(
        process.env.REQUIRE_GOOGLE_ROUTES_FOR_PRICING ?? "true",
      ).toLowerCase() === "true";
    ...
    res.json({
      ...
      quote: {
        ...
        caretaker: {
          ...
          rating: caretaker.averageRating,
        },
        distanceKm: route.distanceKm,
...
async function createBooking(req, res, next) {
  try {
    ...
    if (!parent || !hospital || !caretaker)
      return res
        .status(400)
        .json({ success: false, message: "Invalid booking selection" });
    ...
    const conflict = await Booking.findOne({
      ...
      startTime,
      status: {
        $in: ["requested", "accepted", "in_progress", "payment_pending"],
      },
    });
    if (conflict)
      return res
    ...
    await notify(
      canonicalCaretakerId,
      "New Booking Request",
      `${clientName} requested a hospital visit booking for ${parent.fullName}.`,
      "booking_requested",
    );
    await notify(
      req.user._id,
      "Booking Requested",
      "Your caretaker booking request was sent successfully.",
      "booking_requested",
    );

    res
      .status(201)
      .json({ success: true, booking: await getBooking(booking._id) });
  } catch (error) {
    next(error);
  }
}
...
async function listCaretakerBookings(req, res, next) {
  try {
    res.json({
      success: true,
      bookings: await Booking.find({ caretakerId: req.user._id })
        .sort({ createdAt: -1 })
        .populate("parentId hospitalId")
...
async function getBookingById(req, res, next) {
  try {
    ...
    const allowed = [String(clientUserId), String(caretakerUserId)].includes(
      String(req.user._id),
    );
...
async function updateBookingStatus(req, res, next) {
  try {
    ...
    if (req.user.role === "caretaker") {
      if (String(booking.caretakerId) !== String(req.user._id))
        return res
          .status(403)
      ...
      if (
        ...
        return res
          .status(400)
          .json({ success: false, message: "Invalid booking transition" });
      booking.status = status;
      await booking.save();
      await notify(
        booking.clientId,
        status === "accepted" ? "Booking Accepted" : "Booking Rejected",
        `Your caretaker has ${status} your booking request.`,
        `booking_${status}`,
      );
      ...
      if (
        ...
        await notify(
          booking.caretakerId,
          "Booking Cancelled",
          "A client cancelled the booking request.",
          "booking_cancelled",
        );
      } else
        return res
...
async function updateProgress(req, res, next) {
  try {
    ...
    if (booking.status !== "in_progress")
      return res
        .status(400)
        .json({ success: false, message: "The booking is not in progress" });
    ...
    booking.progress.stages = booking.progress.stages.map((stage, i) => {
      ...
      if (i < idx)
        return {
          ...
          updatedBy: s.updatedBy || req.user._id,
        };
      if (i === idx)
        return {
          ...s,
    ...
    if (nextStage === "task_completed") {
      booking.progress.stages[booking.progress.stages.length - 1].status =
        "completed";
    ...
    await notify(
      booking.clientId,
      `Care Progress: ${booking.progress.stages.find((s) => s.key === nextStage)?.label}`,
      `Your caretaker updated the hospital visit progress to ${booking.progress.stages.find((s) => s.key === nextStage)?.label}.`,
      "care_progress",
    );
    res.json({ success: true, booking: await getBooking(booking._id) });
  } catch (e) {
...
async function clientComplete(req, res, next) {
  try {
    ...
    if (!booking || String(booking.clientId) !== String(req.user._id)) {
    ...
    if (
      ...
      booking.progress.stages.every(
        (s) => s.key !== "task_completed" || s.status !== "completed",
      )
    ) {
      return res.status(400).json({
        success: false,
        message: "Caretaker must complete the task first",
      });
    }

    /*
     * ----------------------------------------------------------
     * Reuse an existing payment when available.
     * This also repairs older bookings that reached
     * payment_pending before the payment record was created.
     * ----------------------------------------------------------
     */
    ...
    /*
     * ----------------------------------------------------------
     * Update completion/payment state only after the payment
     * record is successfully available.
     * ----------------------------------------------------------
     */

    booking.clientCompletedAt = booking.clientCompletedAt || new Date();
    ...
    await notify(
      ...
      "The client also confirmed task completion. Payment is now pending.",
      "payment_pending",
    );
    ...
    if (hasCoords) {
      ...
      if (discovered.length) {
        ...
        for (const item of discovered) {
          ...
          const doc = await Hospital.findOneAndUpdate(filter, update, {
            new: true,
            upsert: true,
      ...
      return res.json({
        ...
        searchSource: discovered.length
          ? discovered[0].source
          : "local_database",
      });
    }
    ...
    if (!rows.length) {
      const defaults = [
        ...
        },
        {
          ...
          address: "Kuliyapitiya, Kurunegala District, Sri Lanka",
          district: "Kurunegala",
          town: "Kuliyapitiya",
          location: { lat: 7.47131, lng: 80.04481 },
          isActive: true,
        },
        {
          ...
          district: "Kurunegala",
          town: "Kuliyapitiya",
          location: { lat: 7.472043, lng: 80.044601 },
          isActive: true,
        },
      ];
...
async function geocode(req, res, next) {
  try {
    ...
    res.json({
      success: true,
      location: await geocodeAddress(`${query}, Sri Lanka`),
    });
  } catch (e) {
    next(e);
  }
...
async function routeTest(req, res, next) {
  try {
    ...
    if (
      ![origin.lat, origin.lng, destination.lat, destination.lng].every(
        Number.isFinite,
      )
    ) {
      return res
...
async function createHospital(req, res, next) {
  try {
    const row = await Hospital.create(req.body);
    res.status(201).json({ success: true, hospital: row });
  } catch (e) {
    next(e);
  }
}
