...
const { getPricingSettings } = require("../services/pricingService");
...
async function recommend(req, res, next) {
  try {
    ...
    const caretakerServiceCharge = Number(
      pricingSettings?.caretakerServiceCharge || 0,
    );
    ...
    for (const c of caretakers) {
      ...
      const ratingScore = Math.max(
        0,
        Math.min(100, (c.averageRating || 0) * 20),
      );
      const completedServices = await Booking.countDocuments({
        caretakerId: c.userId?._id || c.userId,
        status: { $in: ["paid", "closed"] },
      });
      const completedScore = Math.min(100, completedServices * 10);
      const previousInteraction = (await Feedback.exists({
        caretakerId: c.userId?._id || c.userId,
      ...
      out.push({
        ...
        factors: {
          ...
          rating: ratingScore,
          completedServices: completedScore,
          previousInteraction,
        },
        distanceKm,
