  ...
  useEffect(() => {
    ...
    const loadStatistics = async () => {
      try {
        ...
        setStats({
          familiesHelped: Number(data.familiesHelped ?? 0),
          verifiedCaretakers: Number(data.verifiedCaretakers ?? 0),
          averageRating: Number(data.averageRating ?? 0),
        });
      } catch (error) {
        console.error("Failed to load CareLink+ public statistics:", error);
          ...
          <div className="grid lg:grid-cols-2 gap-14 items-center">
            ...
            <div>
              ...
              <h1 className="mt-8 text-5xl lg:text-6xl font-bold leading-tight tracking-tight text-slate-900">
                ...
                <br />
                for a{" "}
                <span className="text-[#003898]">healthier tomorrow.</span>
              </h1>

              <p className="mt-8 max-w-xl text-xl leading-9 text-slate-600">
                CareLink+ was built with one simple mission—to give every Sri
                Lankan family complete peace of mind. We connect families with
                trusted, verified caretakers who provide compassionate
                assistance for hospital visits, elderly care, and everyday
                support whenever it&apos;s needed most.
              </p>
