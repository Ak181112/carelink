...
import { Banknote, Briefcase, CheckCircle2, Clock3, AlertCircle, CalendarDays } from "lucide-react";
...
export default function CaretakerDashboardPage() {
  ...
  useEffect(() => {
    Promise.all([
      caretakerAPI.getMyProfile().then((d) => {
        setProfile(d.profile);
        setAvailable(d.profile?.isAvailable ?? false);
      }).catch(() => {}),
      notificationAPI.getAll().then((d) => setUnread(d.unreadCount || 0)).catch(() => {}),
  ...
  const status = profile?.applicationStatus || "not_applied";
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="mt-1 text-[#42526E]">Manage your caretaker profile and availability.</p>
      </div>
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-xs font-medium text-[#42526E] uppercase tracking-wide">Application Status</p>
            <span className={`mt-1 inline-block rounded-full px-3 py-1 text-sm font-bold ${statusInfo.color}`}>
              {statusInfo.label}
            </span>
          </div>
        </div>
        {status === "not_applied" && (
          <p className="mt-2 text-sm text-[#42526E]">
            Complete your profile and submit an application to start receiving care requests.
          </p>
        )}
        {status === "rejected" && (
      ...
      {/* Availability Status — approved caretakers only */}
      {isApproved && (
        ...
        {[
          {
            ...
            color: profile ? "bg-green-50 text-green-600" : "bg-yellow-50 text-yellow-600",
          },
          {
            label: "Application",
          ...
          <Link
            key={stat.label}
            href={stat.href}
            className="bg-white rounded-2xl p-6 border border-[#DFE1E6] hover:shadow-md transition"
          >
            <div className={`inline-flex h-12 w-12 items-center justify-center rounded-xl ${stat.color} text-2xl mb-4`}>
              {stat.icon}
            </div>
            <p className="text-lg font-bold text-[#091E42]">{loading ? "—" : stat.value}</p>
            <p className="mt-1 text-sm text-[#42526E]">{stat.label}</p>
          </Link>
        ))}
      </div>

      {/* Earnings + Available Jobs */}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div className="bg-white rounded-2xl border border-[#DFE1E6] p-6">
          ...
          <div className="space-y-2">
            <div className="flex justify-between py-2.5 border-b border-[#DFE1E6]">
          ...
          <p className="mt-3 text-xs text-gray-400">Earnings appear once bookings are completed.</p>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-[#DFE1E6] p-6">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h2 className="text-base font-bold text-[#091E42]">Available Jobs</h2>
          </div>

          {!isApproved ? (
            <div className="flex items-start gap-3 rounded-xl bg-yellow-50 p-4">
              <AlertCircle className="h-4 w-4 text-yellow-600 mt-0.5 shrink-0" />
              <p className="text-sm text-yellow-700">Get approved to receive job requests from clients.</p>
            </div>
          ) : (
      ...
      {/* Quick Actions */}
      <div className="bg-white rounded-2xl border border-[#DFE1E6] p-6">
        <h2 className="text-base font-bold text-[#091E42] mb-4">Quick Actions</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <Link
            href="/caretaker/profile"
          ...
          >
            ...
            <p className="text-xs text-[#42526E] mt-1">Edit skills, experience, and photo</p>
          </Link>
          {status !== "pending" && (
            <Link
              href="/caretaker/application"
            ...
            >
              ...
              <p className="mt-2 font-semibold text-[#091E42]">
                {status === "not_applied" ? "Submit Application" : "View Application"}
              </p>
              <p className="text-xs text-[#42526E] mt-1">Upload documents for admin review</p>
            </Link>
          )}
        </div>
      </div>
